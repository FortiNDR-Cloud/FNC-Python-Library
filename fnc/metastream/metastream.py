...
def _validate_start_date(start_date: datetime, checkpoint: datetime):
    if start_date > checkpoint:
        raise InputError(
            message="start_date must be at least 1 second in the past")
...
def _validate_event_types(event_types):
    if not all(e in EVENT_TYPES for e in event_types):
        raise InputError(
            f'event_types must be of the following: {", ".join(EVENT_TYPES)}')
...
def _prefix_to_datetime(date_prefix: str) -> datetime:
    ...
    try:
        ...
        raise ServerError(
            message=f'unknown format for date_prefix: {date_prefix}', code=0) from None
...
def _get_bucket_or_default(bucket: str, env: str):
    if bucket:
        return bucket
    else:
        e = env if env else 'production'
        return DEFAULT_BUCKET[e]
...
def fetch_events(name: str, event_type: str, account_code: str = None, api_token: str = None,
                 start_date: datetime = datetime.now(
                     timezone.utc) - timedelta(minutes=5),
                 access_key: str = None, secret_key: str = None, limit: int = 0, bucket: str = None, env: str = "production",
    ...
    with S3Client(_get_bucket_or_default(bucket, env), access_key, secret_key, user_agent_extra, context=context) as s3:
...
def fetch_events_by_day(name: str, day: datetime, event_type: str, account_code: str, api_token: str = None,
    ...
    account_code = _fetch_account_code(env, account_code, api_token)
    _validate_event_types([event_type])
...
def fetch_detections_by_day(name: str, day: date, account_code: str, access_key: str = None, secret_key: str = None,
                            limit: int = 0, bucket: str = None, env: str = "production", context: Context = None) -> List[dict]:
    """fetches events from metastream for an entire day.  See README.md for full details"""
    _validate_day(day)
...
def fetch_detections(name: str, account_code: str, start_date: datetime, access_key: str = None, secret_key: str = None,
                     limit: int = 0, bucket: str = None, env: str = "production", context: Context = None):
    """fetches events from metastream.  See README.md for full details"""
    checkpoint = datetime.now(tz=timezone.utc).replace(microsecond=0)
    if context:
        context.checkpoint = checkpoint
